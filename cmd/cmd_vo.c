...
{
    if (!((intftype & VO_INTF_MIPI) || (intftype & VO_INTF_MIPI_SLAVE))) {
        return;
    }
...
{
    if ((vo_get_interface_type(dev) & VO_INTF_MIPI) ||
        (vo_get_interface_type(dev) & VO_INTF_MIPI_SLAVE)) {
        /* stop mipi */
#if CONFIG_MIPI_TX_SUPPORT
        mipi_tx_stop(VO_INTF_MIPI);
#endif
    }
}
...
static int start_layer_parse(char *const argv[], unsigned int *layer,
    unsigned long *addr, unsigned int *strd, rect *layer_rect)
{
    int ret;
