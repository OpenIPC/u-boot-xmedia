...
static void xm72050300_io_config_spi_sfc(void)
{
...
static void xm72050300_io_config_spi_sdio(void)
{
	/* set pad ctrl reg for spi */
...
void fmc_set_fmc_system_clock(struct spi_op *op, int clk_en)
{
	...
	if (op && op->clock) {
		...
		if (spi_input_sle(sysstat_val))
			xm72050300_io_config_spi_sdio();
		else
			xm72050300_io_config_spi_sfc();
	}
	if (clk_en)
...
void fmc_get_fmc_best_2x_clock(unsigned int *clock)
{
	...
	for (ix = 0; (sys_2x_clk[ix] && ((ix + 1) < sizeof(sys_2x_clk))); ix += _2B) {
